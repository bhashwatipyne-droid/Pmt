...
import { getClients, createClient, getProjects } from "@/services/api";
...
const AddClientModal = ({ open, onClose, onCreated }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-6" onClick={onClose}>
      <div data-testid={CLIENTS.addModal} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input data-testid={CLIENTS.addModalName} value={name} onChange={(e) => setName(e.target.value)} className={inputBase} placeholder="e.g. AMFI" />
          </div>
          <div>
            <label className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-slate-500">Contact Person</label>
            <input data-testid={CLIENTS.addModalContact} value={contact} onChange={(e) => setContact(e.target.value)} className={inputBase} placeholder="Full name" />
          </div>
        </div>
        <div className="mt-5 flex justify-end gap-2">
          <button data-testid={CLIENTS.addModalCancel} onClick={onClose} className="rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Cancel</button>
          <button data-testid={CLIENTS.addModalSubmit} onClick={handleSubmit} disabled={submitting} className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60">
    ...
    <div data-testid={CLIENTS.page} className="flex-1 overflow-auto px-8 py-6">
      <div className="mb-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button data-testid={CLIENTS.addBtn} onClick={() => setModalOpen(true)} className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3.5 py-2 text-sm font-semibold text-white hover:bg-slate-800">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={c.id} data-testid={`${CLIENTS.rowPrefix}-${c.id}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                ...
                <td className="px-4 py-3">
                  <button className="rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700" title="Edit coming soon">
      ...
      <AddClientModal open={modalOpen} onClose={() => setModalOpen(false)} onCreated={fetchAll} />
    </div>
  );
}
